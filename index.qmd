...
happy.res <- rbind("$$\\textrm{Valores singulares}/\\sqrt{p}$$" = happy.pca$sv*sqrt(5), happy.cor, "$$\\textrm{SS}$$" = happy.cor |> apply(2, FUN = function(x) sum(x^2)))
happy.res[8,] <- happy.res[7,]/5 * 100
row.names(happy.res)[8] <- "$$\\%$$"
happy.res <- cbind(happy.res, "SS" = c(round(happy.res[1:6,] |> apply(1, FUN = function(x) sum(x^2)),0),5,100))

happy.res |> round(3) |> gt(rownames_to_stub = T) |> tab_options(table.font.size = 11) |> data_color(2:3,palette = c("lightskyblue1")) |> 
  tab_row_group(
  ...
  row_group_order(groups = c("Valores singulares", "Correlaciones entre las componentes principales y las variables originales","Variabilidad explicada")) |> 
    tab_style(
    style = list(cell_text(weight = "bold", size = "normal")
      ),
    locations = cells_row_groups()
  ) |> fmt_markdown(columns = everything()) |>   fmt_percent(
    rows = 8,
    decimals = 2,
    sep_mark = ',',
    scale_values = FALSE
